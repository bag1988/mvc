...
function saveNewDirectory() {
    ...
    ajax({
        ...
        success: function (data) {
            loadImagesUser('', 'viewDir');
            viewMessage('ErrorNewDir', (data));
        }
    });
}
var urlActiv = '';
function loadImagesUser(urlDir, idDiv) {
    var model = new FormData();
    ...
    ajax({
        ...
        success: function (data) {
            ...
            if (urlDir) {
                div.innerHTML = '<ol class="breadcrumb"><li><a href="#" onclick="loadImagesUser(\'\',\'' + idDiv + '\')">Images</a> / </li><li class="active">' + urlDir + '</li></ol>';
            }
            else {
            ...
            data.nameDirectory.forEach(function (d, i) {
                ...
                a.className = 'abtn';
                a.innerText = d;
                a.onclick = function () { loadImagesUser(this.innerText, idDiv); };
            ...
            data.nameFile.forEach(function (c, i) {
                ...
                bag(d).addHtml('<div><img class="img-thumbnail" src="' + c + '"><div class="caption"><p></p></div></div>');
                if (idDiv != 'viewImagesDiv') {
                    var a = document.createElement('a');
                    a.className = 'abtn';
                    a.innerText = 'Удалить';
...
function saveNewImages() {
    ...
    ajax({
        ...
        data: model,
        success: function (data) {
            loadImagesUser(urlActiv, 'viewDir');
            viewMessage('ErrorNewDir', (data));
        }
    });
}
...
function showViewModule(idBlock) {
    ajax({
        url: '/admin/exensions/viewModule',
        success: function (data) {
